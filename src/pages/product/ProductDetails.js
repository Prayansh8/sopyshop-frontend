...
import {
  clearErrors,
  getProductDetails,
  newReview } from "../../redux/actions/productAction";
...
import { Add, Remove, ShoppingCart, ErrorOutline } from "@mui/icons-material";
  ...
  if (!loading && error && !product) {
      return (
          <Box sx={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 2 }}>
              <ErrorOutline sx={{ fontSize: 60, color: 'error.main', opacity: 0.5 }} />
              <Typography variant="h4" fontWeight={800}>Product Not Found</Typography>
              <Typography color="text.secondary">The product you're looking for might have been removed or doesn't exist.</Typography>
              <Button variant="contained" href="/products" sx={{ mt: 2, borderRadius: 3 }}>Browse Other Products</Button>
          </Box>
      );
  }
    ...
    <Fragment>
      {loading ? (
        ...
        <Box sx={{ bgcolor: "background.default", py: 6, minHeight: "100vh" }}>
          <Metadata title={`${product?.name || 'Product'} | Sopyshop`} />
          <Box maxWidth="lg">
            <Grid container spacing={6} columns={12}>
              {/* Product Images */}
              <Grid item xs={12} md={6} lg={6} sx={{width: '45%'}}>
                <Paper 
                  elevation={0} 
                  sx={{ 
                    p: 2, 
                ...
                >
                  <Carousel 
                    ...
                    sx={{ width: '100%', height: { xs: 350, md: 500 } }}
                  >
                    {product.images &&
                      product.images.map((item, i) => (
                        ...
                        >
                          <img
                            src={item.url}
                            alt={`${i} Slide`}
                            style={{ width: "100%", height: "100%", minWidth: "100%", maxWidth: "100%", minHeight: "100%", maxHeight: "100%", objectFit: "contain", borderRadius: "12px" }}
                          />
                        </Box>
                      ))}
                  </Carousel>
              ...
              {/* Product Info */}
              <Grid item xs={6} md={6} lg={6} sx={{mt:5,width: '50%'}}>
                <Stack spacing={3}>
                  <Box>
